...
void OLED::begin() {
  i2c1.write(OLED_ADDR, OLED_COMMAND, oled_init, sizeof(oled_init));
  wait(300);
  clrScreen();
}
...
void OLED::senddata(uint8_t data) {
  i2c1.write(OLED_ADDR, OLED_DATA, data);
}

// send zeros
void OLED::sendzeros(uint8_t nbytes) {
  i2c1.writezeros(OLED_ADDR, OLED_DATA, nbytes);
}

// send ones
void OLED::sendones(uint8_t nbytes) {
  i2c1.writeones(OLED_ADDR, OLED_DATA, nbytes);
}

// turn off the display
void OLED::noDisplay() {
  i2c1.write(OLED_ADDR, OLED_COMMAND, OLED_OFF);
}

// turn on the display
void OLED::onDisplay() {
  i2c1.write(OLED_ADDR, OLED_COMMAND, OLED_ON);
}
...
void OLED::setPage(uint8_t x, uint8_t y) {
  ...
  (0x10 | ((x & 0xf0) >> 4)),
  (x & 0x0f)};
  i2c1.write(OLED_ADDR, OLED_COMMAND, data_arr, 3);
}
...
void OLED::showCursor(uint8_t x) {
  if (x) {
    setPage(oledX+2, 7);
    sendones(4);
  } else {
